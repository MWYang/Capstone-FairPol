...
def main(data_dir='../data/'):
    ...
    if not os.path.isfile(target):
        ...
        with open(target, 'wb') as f:
            tqdm_obj = tqdm(r.iter_content(block_size),
    ...
    chicago = chicago[
        sp.logical_and(2012 <= chicago['Year'], chicago['Year'] <= 2017)
    ]
    ...
    meters_height = geopy.distance.distance((MIN_LAT, MIN_LON), (MAX_LAT, MIN_LON)).m
    meters_width = geopy.distance.distance((MIN_LAT, MIN_LON), (MIN_LAT, MAX_LON)).m
    ...
    chicago_small = chicago[
        sp.logical_and(2014 <= chicago['Year'], chicago['Year'] <= 2015)
    ]
